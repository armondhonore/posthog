"""Business logic for reading and writing per-account custom property values.

The single write path (`set_custom_property_value`) owns type validation/coercion, the soft-delete +
insert transaction, and the append-only history invariant — so the DRF presentation and (later) the
Max tool share identical behavior. Callers pass `team_id`; this module is framework-free and raises
domain errors that the presentation layer maps to HTTP.
"""
