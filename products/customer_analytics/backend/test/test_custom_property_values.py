...
class TestSetCustomPropertyValue(BaseTest):
    ...
    def test_unknown_definition_raises(self):
        with pytest.raises(CustomPropertyDefinition.DoesNotExist):
            set_custom_property_value(
...
class TestCustomPropertyValueFacade(BaseTest):
    ...
    @parameterized.expand(
        [
            ("text", DisplayType.TEXT, "enterprise", "enterprise", "string"),
            ("number", DisplayType.NUMBER, 9.99, 9.99, "numeric"),
            ("boolean", DisplayType.BOOLEAN, True, True, "boolean"),
            (
                "datetime",
                DisplayType.DATETIME,
                "2026-01-01T12:00:00Z",
    ...
    def test_set_returns_a_contract_with_the_typed_value_and_data_type(
        self, _name, display_type, value, expected_value, data_type
    ):
