    ...
    *,
    ...
    for attempt in range(_MAX_LLM_ATTEMPTS):
        ...
        if function_calls(response):
            # Tool budget spent mid-tool-use: this response carries an unanswered `function_call`. Fail the step
            # rather than carry a dangling call forward (Gemini rejects a call with no matching response).
            REPLAY_VISION_PROVIDER_CALL.labels(**metric_labels, outcome="tool_budget_exhausted").observe(
                time.monotonic() - started
            )
            logger.warning(
                "replay_vision.call_scanner_provider.tool_budget_exhausted", step=step.name, required=step.required
            )
            return None
