...
@pytest.mark.asyncio
async def test_step_fails_when_tool_budget_exhausted_mid_tool_use() -> None:
    # The model never stops calling the tool: budget runs out with an unanswered call, so the step must fail
    # outright rather than carry a dangling call forward.
    steps = [MissionStep(name="core", instruction="c", response_model=_Core, required=False)]
    # initial generate + 6 tool iterations = 7 responses, every one a function call.
    ...
    assert len(client.models.calls) == 7  # no re-prompt
